...
export function LeadsKanban({ leads, stages }: { leads: LeadType[], stages: StageType[] }) {
  return (
    <div className="flex-1 flex gap-4 overflow-x-auto pb-4 h-[calc(100vh-16rem)] min-h-[500px]">
      {stages.map((stage) => {
        ...
        return (
          <div key={stage.id} className="flex-shrink-0 w-80 bg-zinc-100 dark:bg-zinc-900/50 rounded-lg flex flex-col">
            ...
            <div className="p-3 flex-1 overflow-y-auto space-y-3">
              {stageLeads.map((lead) => (
                <Link href={`/leads/${lead.id}`} key={lead.id} className="block">
                  <div className="bg-white dark:bg-zinc-900 p-3 rounded-md shadow-sm border border-zinc-200 dark:border-zinc-800 hover:border-blue-500 dark:hover:border-blue-500 transition-colors cursor-pointer">
